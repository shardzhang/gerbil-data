...
class FeatureEncoder4ML1MTest extends WordSpec with Matchers {

  private def createSample(): ML1MTrainSample = {
    val s = new ML1MTrainSample()
    s.user_id = "1"
    s.item_id = "1"
    s.gender = "M"
    s.age = "25"
  ...
  "FeatureEncoder4ML1M" should {
    "setup correctly with all features registered" in {
      val encoder = new FeatureEncoder4ML1M().setup()
