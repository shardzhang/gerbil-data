...
class FeatureEncoderTest extends WordSpec with Matchers {
  ...
  private class TestCategoricalFeature(f_i: Int, f_n: String)
    ...
    override def parse(input: String): RawFeature = {
      clear()
  ...
  private class TestFeatureEncoder extends FeatureEncoder[String] {
    override def setup(): FeatureEncoder[String] = {
      target = new TestRawTarget()
      this
    }
  }

  "FeatureEncoder" should {
    "get_field_name_and_index" in {
      val encoder = new TestFeatureEncoder()
      ...
      val fields = encoder.get_field_name_and_index()
