...
class BaseDataDriverTest extends WordSpec with Matchers {

  // FeatureStat tests
  "FeatureStat" should {
    "add values and track sum/count/powerSum" in {
      val stat = new FeatureStat()
    ...
    "merge two FeatureStats" in {
      val s1 = new FeatureStat(5.0D, 25.0D, 2L)
      val s2 = new FeatureStat(10.0D, 100.0D, 3L)
      s1.merge(s2)
      assert(s1.sum === 15.0D)
  ...
  "PosInfo" should {
    ...
    "merge with FeatureStat" in {
      val info = PosInfo(pos = 1, sum = 5.0D, powerSum = 13.0D, count = 3L)
      val stat = new FeatureStat(3.0D, 5.0D, 1L)
  ...
  // Concrete test driver to test BaseDataDriver methods
  private class TestDriver extends BaseDataDriver[String] {
    override val max_dim: Long = 1000L

    override def feature_encoder: encoder.vectorizer.FeatureEncoder[String] = {
      new encoder.vectorizer.FeatureEncoder[String] {
        override def setup(): encoder.vectorizer.FeatureEncoder[String] = this
      }.setup()
    }
    ...
    def testReadText(reader: BufferedReader): String = readText(reader)
    def testLegacyPosInfo(pos: Int, mean: Double, std: Double, count: Long): PosInfo = legacyPosInfo(pos, mean, std, count)
  }

  "BaseDataDriver.readText" should {
  ...
  "BaseDataDriver.legacyPosInfo" should {
  ...
  "BaseDataDriver.parquet_schema" should {
  ...
  "BaseDataDriver.keepSample" should {
