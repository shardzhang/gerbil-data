...
class UserRateStd7DayContinue(f_i: Int, f_n: String) extends ContinuousFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_std_7day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0.0F }
    raw_list.append(v.toString)
    feature_list.append(1L)
    value_list.append(v)
    this
...
class UserRateStd15DayContinue(f_i: Int, f_n: String) extends ContinuousFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_std_15day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0.0F }
    raw_list.append(v.toString)
    feature_list.append(1L)
    value_list.append(v)
    this
...
class UserRateStd30Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_std_30day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0.0 }
    val buck = v match {
      case x if x <= 0.0 => 1
      case x if x <= 1.0 => 2
      case x if x <= 2.0 => 3
...
class UserRateStd30DayContinue(f_i: Int, f_n: String) extends ContinuousFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_std_30day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0.0F }
    raw_list.append(v.toString)
    feature_list.append(1L)
    value_list.append(v)
    this
...
class UserMovieRateCnt7Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_7day_cnt } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0 }
    val buck = v match {
      case x if x == 0 => 1
      case x if x <= 10 => 2
      case x if x <= 30 => 3
...
class UserMovieRateCnt15Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_15day_cnt } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0 }
    val buck = v match {
      case x if x == 0 => 1
      case x if x <= 10 => 2
      case x if x <= 30 => 3
...
class UserMovieRateCnt30Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_rate_30day_cnt } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 0 }
    val buck = v match {
      case x if x == 0 => 1
      case x if x <= 10 => 2
      case x if x <= 30 => 3
...
class UserAvgRate7Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_avg_rate_7day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 3.0 }
    val buck = v match {
      case x if x == 0.0 => 1
      case x if x < 3.0 => 2
      case x if x < 4.0 => 3
...
class UserAvgRate7DayContinue(f_i: Int, f_n: String) extends ContinuousFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_avg_rate_7day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 3.0F }
    raw_list.append(v.toString)
    feature_list.append(1L)
    value_list.append(v)
    this
...
class UserAvgRate15Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_avg_rate_15day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 3.0 }
    val buck = v match {
      case x if x == 0.0 => 1
      case x if x < 3.0 => 2
      case x if x < 4.0 => 3
...
class UserAvgRate15DayContinue(f_i: Int, f_n: String) extends ContinuousFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_avg_rate_15day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 3.0F }
    raw_list.append(v.toString)
    feature_list.append(1L)
    value_list.append(v)
    this
...
class UserAvgRate30Day(f_i: Int, f_n: String) extends CategoricalFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_avg_rate_30day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 3.0 }
    val buck = v match {
      case x if x == 0.0 => 1
      case x if x < 3.0 => 2
      case x if x < 4.0 => 3
...
class UserAvgRate30DayContinue(f_i: Int, f_n: String) extends ContinuousFeature[ML1MSample](f_i, f_n) {
  override def parse(sample: ML1MSample): RawFeature = {
    val v = try { sample.user_avg_rate_30day } catch { case e: Exception => green_println(s"Featurizer4ML1M parse error: ${e.getMessage}"); 3.0F }
    raw_list.append(v.toString)
    feature_list.append(1L)
    value_list.append(v)
    this
