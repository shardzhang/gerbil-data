...
abstract class Featurizer[T] extends Serializable {
  ...
  def encode(input: T, dim: Long, parquet_builder: ParquetRecordData.Builder, pos_map: collection.Map[(Int, Long), Int], target_map: collection.Map[Int, Int]): (Boolean, Boolean) = {
    for (raw_f <- raw_cate_features) {
      raw_f.clear();
      raw_f.parse(input)
    }
