...
import tfrecords.serde.DefaultTfRecordRowEncoder
  ...
  private def saveLocal(
    ...
    if (dir.exists()) {
      mode match {
        case SaveMode.ErrorIfExists =>
          throw new IllegalStateException(
            s"LocalPath $cleanedPath already exists. SaveMode: ErrorIfExists.")
        case SaveMode.Ignore =>
          return
        case _ =>
      }
    ...
    val f = DefaultSource.writePartitionLocalFun(cleanedPath)
...
object DefaultSource extends scala.Serializable {
  ...
  private def writePartitionLocalFun(
      localPath: String): (Int, Iterator[(BytesWritable, NullWritable)]) => Iterator[Int] = {
    def mapFun(index: Int, part: Iterator[(BytesWritable, NullWritable)]) = {
      writePartitionLocal(index, part, localPath)
    }
    mapFun
  }
